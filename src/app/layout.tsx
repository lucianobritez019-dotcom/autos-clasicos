...
import { buildWhatsAppLinkText, hasWhatsAppNumber, getSettings } from "@/lib/datasource";
...
export const metadata: Metadata = {
  title: "Clásicos Seleccionados",
  description:
    "Exhibición de autos usados clásicos. Muestras detalladas, fotos y videos, y contacto directo por WhatsApp.",
  metadataBase: new URL("https://autos-clasicos.preview.same-app.com"),
  openGraph: {
    title: "Clásicos Seleccionados",
    description:
      "Exhibición de autos usados clásicos. Muestras detalladas, fotos y videos, y contacto directo por WhatsApp.",
    images: [
      {
        url: "https://images.unsplash.com/photo-1604940500627-d3f44d1d21c6?auto=format&fit=crop&q=80&w=1200",
        width: 1200,
        height: 630,
        alt: "Autos Clásicos",
      },
...
export default async function RootLayout({
  ...
  const showWhatsApp = await hasWhatsAppNumber();
  const whatsappLink = showWhatsApp
    ? await buildWhatsAppLinkText("Hola, estoy interesado en un vehículo clásico.")
    : null;
        ...
        <ClientBody>
          <header className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/60 bg-white/80 border-b">
            <div className="container flex h-14 items-center justify-between">
              <Link href="/" className="font-semibold tracking-wide">
                {settings.siteTitle || "Clásicos"}
              </Link>
              <nav className="text-sm text-muted-foreground">
                <a href="#vendidos" className="hover:text-foreground">Vendidos</a>
              </nav>
            </div>
          </header>
          {children}
          <footer className="border-t mt-8">
            <div className="container py-8 text-sm text-muted-foreground">
              © {new Date().getFullYear()} {settings.siteTitle || "Clásicos Seleccionados"} — Exhibición.
            </div>
          </footer>
          {showWhatsApp && (
            <a
              ...
              className="fixed bottom-6 right-6 z-[60] inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-600 text-white shadow-lg hover:bg-emerald-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
            >
